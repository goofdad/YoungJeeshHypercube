...
//gear2_bolt_sink          = 4; // bolt
gear2_bolt_sink          = 6; // nut
...
module gearsbyteethanddistance(t1=13,t2=51, d=60, teethtwist=1, which=1)
{
	...
	//		render()
			...
			//shafthole
			translate([0,0,-TT]) 
				cylinder(r=gear1_shaft_r, h=gear_h+gear_shaft_h+ST, $fn=100);

			//setscrew shaft
			translate([0,0,gear_h+gear_shaft_h-gear1_setscrew_offset])
				rotate([0,90,0])
				cylinder(r=gear1_setscrew_r, h=g1p_d + (cp / 180), $fn=50);
